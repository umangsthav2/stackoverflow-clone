...
export default function Login() {
    ...
    const handleLogin = (e) => {
        e.preventDefault();
        const loginAPI = new Promise((ok, fail) => {
            
            if(username==='durga' && password==='durga@123')
                {
                    sessionStorage.setItem("authToken","CLB3938F31");
        ...
        <>
            <div className="login-page  h-[90vh] flex flex-col justify-center items-center bg-[#f0f0f0]">
                <form onSubmit={handleLogin} noValidate className='flex shadow-md  flex-col rounded-2xl w-[400px] h-[400px] p-10 bg-white' action="">
                    <Link to='/'><img className={`h-[60px] text-center`} src={SOMiniLogo} alt="" /></Link>

                    <div className="mb-2 flex flex-col  input-group">
                        <label className='p-2 font-bold text-xl' htmlFor="email">Email</label>
                        <input onChange={handleUsername} value={username} className='p-2 rounded-md border-2 bg-[#f0f0f0]' type="email" name="" id="email" />
                    </div>
                    <div className="mb-10 flex flex-col input-group relative">
                        ...
                        <input onChange={handlePasword} value={password} className='p-2 rounded-md border-2 bg-[#f0f0f0]' type={inputType} name="" id="password">
                        </input>
